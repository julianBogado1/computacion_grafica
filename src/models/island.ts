...
import {ElevationGeometry} from '../utils/elevationGeometry.ts';
...
export async function createIsland(): Promise<THREE.Group> {
    ...
    // Color texture (separate from heightmap)
    const colorTexture = textureLoader.load('textures/island/island_texture_gpt.png'); 
    ...
    for (let i = 0; i < positions.count; i++) {   
      ...
      let normalizedY = Math.min(1, y / amplitude); // 0 = sea level, 1 = peak

      const t = Math.max(0, normalizedY);                                                                                        
    ...
    const airportAxes = new THREE.AxesHelper(100);
    airport.add(airportAxes);
...
function terrainColor(t: number): THREE.Color {                                                                                                                             
  ...
  if (t < grassHeight) {
    return new THREE.Color(sand).lerp(lightGrass, t / grassHeight);               // sand → bright green
  } else if (t < forestHeight) {
    return new THREE.Color(lightGrass).lerp(midGrass, (t - grassHeight) / (forestHeight - grassHeight));  // bright → mid green
  } else if (t < snowHeight) {
    return new THREE.Color(midGrass).lerp(darkGrass, (t - forestHeight) / (snowHeight - forestHeight));  // mid → dark green
  } else {
    return new THREE.Color(darkGrass).lerp(snow, (t - snowHeight) / (1 - snowHeight));        // dark green → snow
  }
}
