...
export const hangarDefaultProps: HangarProps = {
  hangarLength: 200,
  hangarHeight: 50,
};

export function createHangar({ hangarLength, hangarHeight } = hangarDefaultProps): THREE.Mesh {
  const radius = hangarHeight;

  const hangarTexture = new THREE.TextureLoader().load('textures/airport/metal_tank.webp');
