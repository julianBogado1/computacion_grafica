...
const GRAVITY          = 9.81;
const SHOT_COOLDOWN    = 0.25; // seconds between shots
const PROJECTILE_SPEED = 200;
const PROJECTILE_MASS  = 10;
...
export function fireProjectile(projectiles: Projectile[], cannon: THREE.Object3D): THREE.Mesh {
  ...
  const mesh = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({ color: 0xffcc44, emissive: 0x663300 }),
  );
  mesh.position.copy(pos);

  const vel = dir.clone().multiplyScalar(PROJECTILE_SPEED);
  projectiles.push({ mesh, vel, spawnTime: now, mass: PROJECTILE_MASS });
...
/**
 * Create a bounding box for every target and check if the projectile's position is inside any of them.
 * If so, call `onHit()`.
 */
