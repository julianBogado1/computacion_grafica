...
export function ElevationGeometry(width: number, height: number, amplitude: number, widthSegments: number, heightSegments: number, texture: THREE.Texture)
	...
	for (let i = 0; i < canvas.width - 1; i++) {
		for (let j = 0; j < canvas.height - 1; j++) {
            ...
            if (i > 0 && xPrev == undefined || i < widthSegments - 1 && xNext == undefined ||
                ...
                console.error("Valor de pixel adyacente indefinido");
                return null;
            }
			let deltaX;
			if (xPrev == undefined) {
